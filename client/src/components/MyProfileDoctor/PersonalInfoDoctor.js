...
import "./PersonalInfoDoctor.css";
import { changeColorBox } from "../../usefulFunc/changeColorBox";
export default function PersonalInfoDoctor() {
  ...
  const handleSubmit = () => {
    ...
    fetch("http://localhost:3001/doctor/updatePersonalInfo", {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
      ...
      {toEdit == false && (
        <>
          ...
          <Box>
            ...
            <Typography variant="caption">Unitate sanitară</Typography>
            <Box
              sx={{
                borderColor: "#dc4c4c",
                borderStyle: "solid",
      ...
      {toEdit == true && (
        <>
          ...
          <Box>
            ...
            >
              <Input
                defaultValue={persInfo.sanitaryUnit}
                className={classes.inputForm}
                onChange={(e) => {
                  if (e.target.value.trim().length > 2)
                    setPersInfo({ ...persInfo, sanitaryUnit: e.target.value });
